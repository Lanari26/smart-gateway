...
// Hosted-checkout endpoints — a paying customer has no merchant session, so the
// whole router is public. The merchant console reads results via /transactions.
...
paymentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    res.status(201).json(await service.createMomoCharge(req.body));
  }),
);
...
paymentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    res.status(201).json(await service.createCardCharge(req.body));
  }),
);
